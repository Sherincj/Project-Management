...
const StatCard = ({ title, value, icon }) => {
  return (
    <div className="bg-[#4C3575] my-3 md:my-4 lg:my-6 text-white p-2 md:p-4 rounded-lg flex items-center">
...
const DashboardLayout = ({ children }) => {
  return (
    <div className="bg-gray-100 min-h-screen flex flex-col">
      <FixedHeader />
      <main className="flex-1 flex justify-end p-2 md:p-4 lg:p-6">
        <div className="max-w-7xl w-full lg:w-4/5 xl:w-3/4">
          {children}
        </div>
      </main>
    </div>
  ...
  useEffect(() => {
    ...
    const fetchProjects = async () => {
      try {
        ...
        const coming = response.data.filter(project => project.status === 'Not Started').length;
        const progress = response.data.filter(project => project.status === 'Approval').length;
        const finished = response.data.filter(project => project.status === 'Completed').length;
